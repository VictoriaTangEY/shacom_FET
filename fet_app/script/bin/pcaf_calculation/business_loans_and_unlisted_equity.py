...
def _merge_parameter_tables(df: pd.DataFrame, param: Dict) -> pd.DataFrame:
    """
    ...
    # Emission factor: merge on COUNTRY_CODE
    emission_df = get_parameter_data(param, "Emission_Factor")
    ...
    # Industry intensity: merge on INDUSTRY_CLASSIFICATION_CODE
    def _merge_intensity(d: pd.DataFrame, sheet_name: str, value_col: str):
        tbl = get_parameter_data(param, sheet_name)
...
# -----------------------------------------------------------------------------
# Classification: option (1a → 1b → 2a → 3a → 3b) and company emission
# -----------------------------------------------------------------------------
...
def _classify_and_company_emission_vectorized(df: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
    """
    ...
    mask_3a = (opt == NA_STR) & _has_value(rev) & pd.notna(intensity_rev)
    ...
    mask_3b = (opt == NA_STR) & _has_value(ast) & pd.notna(intensity_asst)
...
# -----------------------------------------------------------------------------
# Attribution factor and financed emission
# -----------------------------------------------------------------------------
...
def run_business_loans_and_unlisted_equity(rc, logger, instruments=None, param=None):
    ...
    param = param or {}
    df = instruments.copy()
    df = _merge_parameter_tables(df, param)

    # Step 1: Classify option and company emission (vectorized using merged columns)
    classify_option, company_emission = _classify_and_company_emission_vectorized(df)
    ...
    attribution_factor = _attribution_factor_vectorized(df)
    zero_denom = (
    ...
    for col in ["CLASSIFY_OPTION", "COMPANY_EMISSION", "ATTRIBUTION_FACTOR", "FINANCED_EMISSION"]:
        out[col] = out[col].apply(_to_display_value)
